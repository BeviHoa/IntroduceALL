...
public class BStree {
    ...
    public void Q1_readFile(String fname) throws IOException {
        try (RandomAccessFile f = new RandomAccessFile(fname, "r")) {
            ...
            while (true) {
                ...
                if (!name.substring(0, 1).equals("B") && age < 10) {
                    insert(new Person(name, age));
                }
            }
    ...
    public int avg(Node p) {
        if (p == null) {
            ...
            return sum(p) / (1 + avg(p.left) + avg(p.right));
        }
    }

    public void postOrderQ2(Node p) {
        if (p == null) {
            return;
        }
        postOrder(p.left);
    ...
    void insert(Person x) {
        ...
        while (p != null) {
            ...
            if (p.info.getName().compareToIgnoreCase(x.getName()) > 0) {//p>x->p sang ben phai
                p = p.left;
            } else {
                p = p.right;
            }
        ...
        if (f.info.getName().compareToIgnoreCase(x.getName()) > 0) {//xac dinh x la con trai hay phai cua f
            f.left = new Node(x);
        } else {
            f.right = new Node(x);
        }
    ...
    public void deleteByCopy() {
        ...
        while (q != p) {
            parent = q;
            if (q.info.getName().compareToIgnoreCase(p.info.getName()) > 0) {
                q = q.left;
            } else {
                q = q.right;
            }
...
//    }
//
//    void inOrder(ArrayList<Person> data, Node p) {//sap xep theo thu tu tang dan
//        if (p == null) {
//            return;
//        }
//        inOrder(data, p.left);
...
//    }
//
//    void balance(ArrayList<Person> data, int first, int last) {
//        if (first <= last) {
